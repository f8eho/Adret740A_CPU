...
- Host parser/framing and instrument-program vector tests pass. The single
  PlatformIO firmware succeeds with 1212 bytes RAM / 8192 bytes and 42380
...
8. Implement end-to-end support for the optional frequency doubler: explicit
   persistent option configuration, 560-to-1120-MHz range handling, panel and
   remote behavior, and validation on a chassis actually fitted with the
   module. Do not infer its presence from the write-only instrument bus; see
   [`DETECTION_DOUBLEUR.md`](DETECTION_DOUBLEUR.md).
