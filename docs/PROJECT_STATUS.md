...
- Original 2816 frequency/attenuator lookup grid, transactional two-bank
  EEPROM overlay, cumulative serial calibration commands and safe rollback of
  both corrections and RF configuration on `CAL ABORT` or `Adr RTL`.
- Manual Python calibration assistant with atomic JSON checkpoints, resume,
  2816 dump import and final Flash-table merge tools.
...
- Quantitative 10 Hz step regularity, frequency accuracy and execution of the
  new amplitude-calibration workflow with suitable metrology equipment.
...
- Keyboard increments are stored separately for RF, amplitude, FM, PM and AM;
  ...
  remains pending.
- One firmware now supports the optional frequency doubler through a passive
  startup declaration on Mega D4/PG5: open selects BASE and a jumper to
  `GND_CPU` selects X2. The capability is sampled once before settings are
  loaded and remains immutable until restart; the write-only instrument bus
  is not treated as a presence detector.
- BASE accepts 100 kHz through 560 MHz. X2 accepts 100 kHz through
  1,119,999,990 Hz in 10-Hz steps, using the direct path below 560 MHz, X2/O2
  below 736 MHz and X2/O1 above it. The original EPROM's odd-20000-divisor
  marker was recovered to support the 10-Hz X2 endpoints.
- Panel entry, wheel steps through 1 GHz, serial commands, memories and
  sequences use the startup capability. Structurally valid high-frequency
  memories survive a BASE startup; recall reports E-21 and a sequence stops
  without applying the incompatible entry. `OPT?` reports the selected
  capability and frequency limit.
- Calibration storage version 2 covers 41 x 28 entries and binds each bank to
  BASE or X2. Version-1 migration and profile changes preserve rows 0..28,
  preserve row 29 only for a legacy BASE profile, and neutralize the remaining
  X2 rows. Host tests inject a simulated power loss after every EEPROM write
  of the overlapping legacy-bank migration and verify restart recovery.
- All X2 behavior is validated off-hardware only. Manufacturer degradation
  above 560 MHz (level accuracy, spectral purity, phase noise and FM
  distortion) is documented without narrowing the accepted command ranges.
...
5. Measure frequency accuracy and 10-Hz step regularity with a precision
   counter. Characterize absolute RF amplitude over the complete frequency and
   attenuation range with a calibrated generator, power meter or reference
   analyzer, then execute and validate the manual amplitude-calibration
   workflow.
...
7. Add AUX sequence stepping and replace the zero calibration initializer
   with the table generated by the documented calibration workflow.
8. On a chassis fitted with the optional doubler, validate the D4 declaration,
   560/736-MHz path transitions, 1,119,999,990-Hz endpoint, absolute level,
